...
@section('contents')
  <div class="d-flex flex-column flex-md-row align-items-start align-items-md-center justify-content-between gap-2 mb-3">
        <h1 class="m-0 text-title text-md-left text-center text-md-h4">Laporan Reservasi</h1>
  </div>
  <hr />
  ...
  <!-- Filter Atas -->
  <!-- Tombol Cetak PDF di luar filter -->
<div class="row mb-4 gy-3 flex-column flex-md-row align-items-start align-items-md-center justify-content-between">
  ...
  <div class="col-auto">
    <div class="dropdown">
      ...
      <div class="dropdown-menu p-3" style="min-width: 300px;">
        <form id="filterForm" method="GET" action="{{ route('laporans_reservasi.index') }}">
          ...
          <a href="{{ route('laporans_reservasi.index') }}" class="btn btn-sm btn-secondary w-100 mt-2">Reset</a>
        </form>
      </div>
    </div>
  </div>
  ...
  <div class="col w-100 w-md-auto">
    <form method="GET" action="{{ route('laporans_reservasi.index') }}" id="searchForm">
      <div class="search-wrapper d-flex flex-column flex-sm-row align-items-stretch gap-2 rounded px-3 py-2">
        <div class="d-flex align-items-center w-100">
          ...
          <input
            type="text"
            name="cari"
            class="form-cari text-white w-100"
            placeholder="Cari"
            value="{{ request('cari') }}"
            id="input-cari"
          />
  ...
  <!-- Tombol Cetak PDF -->
  <div class="col-auto">
    <a href="{{ route('laporans_reservasi.cetak', request()->all()) }}" target="_blank" class="btn btn-purple w-100 w-md-auto">Cetak PDF</a>
  </div>
</div>
  ...
  <div class="d-flex justify-content-center mt-3 mb-4">
          @if ($laporan->hasPages())
              {{ $laporan->appends(request()->all())->links('pagination::bootstrap-5') }}
...
$('#input-cari').on('input', function () {
  ...
  }, debounceDelay); // <<<<< penutup setTimeout
});
</script>
